#!/bin/bash
# Builds ClipStack.app — an unsigned-except-ad-hoc bundle you can run locally
...
if [ "$UNIVERSAL" = 1 ]; then
  ...
  BIN=$(swift build -c release --arch arm64 --arch x86_64 --show-bin-path)/ClipStack
else
...
if [ "$INSTALL" = 1 ]; then
  ...
  osascript -e 'quit app "ClipStack"' >/dev/null 2>&1 || true
  for _ in $(seq 1 20); do
    pgrep -f "ClipStack.app/Contents/MacOS/ClipStack" >/dev/null 2>&1 || break
    sleep 0.25
  done
